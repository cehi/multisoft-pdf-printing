...
/****** Object:  StoredProcedure [dbo].[sp_WritePDF]    Script Date: 21/04/2016 8:04:20 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[sp_WritePDF] 
...
BEGIN
	...
	ELSE
		BEGIN
			IF @Filename Like '%Remittance Advice%' OR @Filename Like '%Debtor Statement%'
								
				SET @Cmd = 'rs.exe -i "' + @FRSSScripts + @RSS + '" -s http://localhost/reportserver -e Exec2005 -v vPON="' + LEFT(@PON,6) + '" -v vPrintDate="' + RIGHT(@PON,8) + '" -v vFileName="' + @LocalPath + @FileName + '"'
			ELSE
